...
#ifdef GTA_PS2_STUFF
#ifdef GTA_PC_CONTROLS
	// Direct raw-input fallback for cheat detection. The standard GetTriangleJustDown() etc.
	// rely on PCTempJoyState being populated through the action mapping pipeline, which only
	// sets face button fields when the pressed button matches a configured game action (e.g.
	// VEHICLE_ENTER_EXIT → Triangle). On platforms like Switch, this means raw button presses
	// aren't reliably reflected in NewState for cheat string building. This lambda reads
	// ControlsManager state directly to detect button transitions regardless of action mappings.
